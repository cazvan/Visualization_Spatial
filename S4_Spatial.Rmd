...
# /\/\ this hides messages and warnings
...
```{r, eval=FALSE}
plot(wazipMap,
     border='black',#color of lines
     col='blue', #color of polygon
     lwd=0.1) #thickness
```
...
plot(contriWA_geo, 
     ...
     col = "red", 
     cex = .6, # shrinking
     add=T) # VERY important (put new info on top of map)
```
...
leaflet(contriWA_geo) %>% 
    addTiles() %>% 
    addCircleMarkers(clusterOptions = markerClusterOptions()) #this option clusters the points so it doesn't plot all 3 6 million dots
```
